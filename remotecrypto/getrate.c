...
int main(int argc, char *argv[])
{
    ...
    while ((opt = getopt(argc, argv, "i:o:t:n:s68c")) != EOF)
    {
        ...
        case 'o': /* set output file name */
            ...
            break;
        case 't': /* set time */
            if (1 != sscanf(optarg, "%lli", &timespan))
                return -emsg(3);
            if (timespan <= 0)
                return -emsg(4);
            break;
        case 'n': /* number of rounds */
            if (1 != sscanf(optarg, "%d", &numofrounds))
                return -emsg(8);
            if (numofrounds < 0)
                return -emsg(9);
            break;
    ...
    /* get input buffer */
    ibfraw = (char *)malloc(sizeof(struct rawevent) * BUF_IN_INENVENTS);
    if (!ibfraw)
        return -emsg(10);
    ...
    { /* main loop for reading in data */
        ...
        /* do real read */
        retval = read(inh, &ibfraw[repairidx], (sizeof(struct rawevent) * BUF_IN_INENVENTS) - repairidx); /* read raw events */
        ...
        if (!firstshot)
        { /* load first timing event */
            firstshot = 1;
            re = inbuf[0];
            inbuf = &inbuf[1];
            numret--;
            t0 = ((unsigned long long)re.cv << 17) +
                 ((unsigned long long)re.dv >> 15) + timespan;
            t1 = t0 - 1;
        }
        ...
        for (i = 0; i < numret; i++)
        {
            ...
            if (t1 > t0)
            { /* do output and update new timer */
                /* print result */
                fprintf(outhandle, "%d", cnt[0]); /* total counts */
                /* display whatever detcount was selected */
                for (j = 1; j <= splitoption; j++)
            ...
            dv = inbuf[i].dv;
            if (coincidencecorrection)
            { /* we should do the correction */
                for (j = 1; j < 9; j++)
                    /* no extra cnts taken */
                    if ((dv & cmask[j]) == cmask[j])
                        cnt[j]++;
            }
            else
            { /* no correction */
