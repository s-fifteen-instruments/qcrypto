...
froot=$(ls -tr data/amp-den* | tail -n1 | sed  's/data\///' | sed 's/[0-9]*-[0-9]*.dat$//')
...
for ((density=0; density<8; density+=1)) do
     res=$(cut -f$normal_field --delimiter=" " $datadir/$froot$i-$density.dat | head -n1)
     res1=$(cut -f$blind_field --delimiter=" " $datadir/$froot$i-$density.dat | head -n1)
    echo $i $density $res >>$temp_target0
