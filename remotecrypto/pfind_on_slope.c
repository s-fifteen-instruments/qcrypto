...
void findmax_on_slope(int *buf1, int *buf2, double *maxval, double *sigma,
			 double *mean, int *pos, int size, int ecnt1, int ecnt2)
{
	int i;				   /* counter index */
	double ai, ar, br, bi, or, diff; /* temporary variables */
	double sx, sxx;
	int maxpos;
...
int main(int argc, char *argv[])
{
	...
	findmax_on_slope(buf1_slow, buf2_slow, &maxval_s, &sigma_s, &mean_s, &pos_s,
			zhs, ecnt1, ecnt2);
